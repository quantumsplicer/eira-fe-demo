...
const AadhaarVerifyInfo = ({ showHeading }: AadhaarVerifyInfoProps) => {
  ...
  return (
    <Stack alignItems={"center"} textAlign={"center"}>
      {showHeading && (
        <Typography fontWeight={"bold"}>Aadhaar Verification</Typography>
      )}
      <Typography mt={2} color={"#6F6F6F"}>
        Please keep your Aadhaar egistered phone number with you for
        verification
      </Typography>
      <Typography mt={1} color={"#6F6F6F"}>
        Verify Aadhaar now to increase your payment acceptance limit to{" "}
        <span style={{ color: "#000", fontWeight: "bold", fontSize: "15px" }}>
          ₹50,000
        </span>
      </Typography>
      <Typography mt={1} color="#6F6F6F">
        Limit without Aadhaar Verification -{" "}
        <span style={{ color: "#000", fontWeight: "bold", fontSize: "15px" }}>
          ₹5,000
        </span>
      </Typography>
      ...
      {aadhaarVerificationFailed && (
        <Box
          ...
          borderRadius={3}
          p={2}
          sx={{ backgroundColor: "rgba(251, 203, 168, 0.25)" }}
        >
